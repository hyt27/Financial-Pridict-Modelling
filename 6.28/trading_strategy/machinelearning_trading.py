    ...
    for i, row in whole_predict.iterrows():
        ...
        if today > yesterday:
            ...
            if short_ma > long_ma:
                ...
                # 收盘价卖出
                #balance = balance + buy_num * row['act_close'] - buy_num * row['act_close'] * fees
                #today_yield = (row['act_close'] - row['open']) * buy_num

                sell_price = row['act_close'] - (row['act_close'] * 0.003)  # 扣除千分之三的税费
                balance = balance + buy_num * sell_price - buy_num * row['act_close'] * fees
                today_yield = (sell_price - row['open']) * buy_num

...
def meachinlearning(task_id = '1',stock_id = '000001.SZ',start = '2001-1-1',end = '2024-1-1',modelname = 'linearregression',strategy = 'buy_and_select_when_sell',balance = 100000):
    ...
    plt.legend()
    # plt.show()
    plt.savefig('result/prediction_'+task_id+'.jpg')
    plt.close()


    log_path = 'result/log_'+task_id+'.txt'
    ####strategy
    ...
    plt.savefig('result/return_rate_'+task_id+'.jpg')
    plt.close()
    result = {"predictResult":'result/prediction_'+task_id+'.jpg',
               "returnRate": 'result/return_rate_'+task_id+'.jpg',
               "logInfo":'result/log_'+task_id+'.txt',
               "Return":yield_rate,
